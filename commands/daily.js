...
module.exports.run = (client, message, args) => {
    ...
    if (parseInt(cooldown) < Date.now() / 1000 | 0) {
        user[message.author.id].money += 300000;
        user[message.author.id].dailyclaimed = Date.now() / 1000 | 0;
        fs.writeFile("database.json", JSON.stringify(user), err => {
            if (err) {
                console.log(err)
            } else {
                return message.reply(`You have claimed daily reward of **5,000** ©️  Lock, now you've **${user[message.author.id].money.toLocaleString()}** ©️`);
            }
        })
    } else {
